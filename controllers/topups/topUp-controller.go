...
func TopUp(db *sql.DB, fromId int, toId int, topup entities.TopUps) (int, error, string) {
	var result string
	statementInsertTopup, errTop := db.Prepare("INSERT INTO top_up (from_account_telp , account_telp , amount , created_at)VALUES (?,?,?,?)")
	if errTop != nil {
		return -1, errTop, "error statement"
	}

	resTopUp, err := statementInsertTopup.Exec(fromId, toId, topup.Amount, topup.CreatedAt)
	if err != nil {
...
func ToupUpHistory(db *sql.DB, FromId int, HisTop entities.TopUpHistory) ([]entities.TopUpHistory, error) {
	statementHT, err := db.Query("SELECT u.name_user, t.amount, t.created_at , t.account_telp FROM users u INNER JOIN top_up t ON t.account_telp = u.no_telp WHERE t.account_telp = ? OR t.from_account_telp =?", FromId, FromId)
	if err != nil {
		return nil, err
	}
	...
	for statementHT.Next() {
		...
		var temp int
		errHt := statementHT.Scan(&rowTopupH.NameUser, &rowTopupH.Amount, &rowTopupH.CreatedAt, &temp)
		if errHt != nil {
			return nil, err
		}
